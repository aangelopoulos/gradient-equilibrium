...
def __(__file__, np, os, pd):
    # Read in the data
    experiment = 'gradient_boosting_yhat_ethnicity'
    # Identify all discrete features
    groups = [1,2,3]
    continuous = [0]
...
def __(T, df, np, plt, sns):
    ...
    etas_to_plot = [0, 0.001, 0.01, 0.05]
    plot_every = 10
...
def __(
    ...
    # Make plot of the gradient norms
    _fig, _axs = plt.subplots(nrows=len(groups), ncols=len(kappas_to_plot), figsize=(len(kappas_to_plot)*10,5*len(groups)), sharex="col", sharey=True)
    _t = np.arange(T)+1
