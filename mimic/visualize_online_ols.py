...
def __(__file__, np, os, pd):
    # Read in the data
    experiment = 'gradient_boosting_ols'
...
def __(T, df, np, plt, sns):
    ...
    etas_to_plot = [0, 0.000000001, 0.000000005, 0.00000001]
    plot_every = 10
...
def __(T, d, df, etas_to_plot, kappas_to_plot, np, plot_every, plt, sns):
    ...
    # Make plot of the gradient norms
    _fig, _axs = plt.subplots(nrows=d, ncols=len(kappas_to_plot), figsize=(len(kappas_to_plot)*10,5*d), sharex="col", sharey=False)
    _t = np.arange(T)+1
